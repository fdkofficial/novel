...
class NovelViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        # Show all published novels, or user's own novels (any status)
    ...
    def get_permissions(self):
        if self.action == 'create':
            ...
            # Allow owner or admin to edit/delete
            return [permissions.IsAuthenticated()]
        return [permissions.IsAuthenticatedOrReadOnly()]

    def retrieve(self, request, *args, **kwargs):
        instance = self.get_object()
        # Increment view count only for published novels
    ...
    def perform_create(self, serializer):
        ...
        # Set author_name from user's full name or username
    ...
    def perform_update(self, serializer):
        ...
        if instance.author != self.request.user and not self.request.user.is_staff:
            raise permissions.PermissionDenied("You don't have permission to edit this novel.")
        serializer.save()

    def perform_destroy(self, instance):
        # Only allow owner or admin to delete
        if instance.author != self.request.user and not self.request.user.is_staff:
            raise permissions.PermissionDenied("You don't have permission to delete this novel.")
        instance.delete()
